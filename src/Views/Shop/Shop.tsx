...
export default function Shop({
  ...
  mainAppScrollRef: React.RefObject<HTMLDivElement>;
}) {
  const { setFilteredInventory, filteredInventory } =
  ...
  useEffect(() => {
    if (selectedCategory === "handmade") {
      ...
      const filteredProducts = allProducts.filter(
        (p) => p.category === "Handmade Designs" && !p.sold && !p.hidden
      );
      ...
      const filteredProducts = allProducts.filter(
        (p) => p.category === "Embellished Vintage" && !p.sold && !p.hidden
      );
      if (filteredProducts.length > 0) {
        setFilteredInventory(filteredProducts);
      }
  ...
  useEffect(() => {
    ...
    if (productId) {
      ...
      setShowDetails(false);
      const filtered = allProducts.filter(
        (p) => (!p.hidden || (user && p.hidden)) && !p.sold
      );
  ...
  useEffect(() => {
    ...
    Object.values(reducedSeries).forEach((seriesArr) => {
      seriesArr.sort(
        (a, b) => getDateInMillis(b.createdAt) - getDateInMillis(a.createdAt)
      );
    });
    ...
    const sortedSeries = Object.values(reducedSeries).sort(
      (a, b) =>
        getDateInMillis(b[0].createdAt) - getDateInMillis(a[0].createdAt)
    );
  ...
  function sortInventory(method: string) {
    ...
    const sortedGroups = groupedInventory.map((seriesArr) => {
      ...
      switch (method) {
        case "newest":
          sortedSeries.sort(
            (a, b) =>
              getDateInMillis(b.createdAt) - getDateInMillis(a.createdAt)
          );
          break;
        case "oldest":
          sortedSeries.sort(
            (a, b) =>
              getDateInMillis(a.createdAt) - getDateInMillis(b.createdAt)
          );
          break;
        case "low":
    ...
    sortedGroups.sort((a, b) => {
      switch (method) {
        ...
        case "oldest":
          return (
            getDateInMillis(a[0].createdAt) - getDateInMillis(b[0].createdAt)
          );
        case "low":
  ...
  function handleClearFilter() {
    setIsFiltered(false);
    setShowSortModal(false);
    const allVisibleProducts = allProducts.filter(
      (p) => (!p.hidden || (user && p.hidden)) && !p.sold
    );
  ...
  async function hideProduct(p: IProductInfo) {
    ...
    setAllProducts(
      allProducts.map((product) =>
        product.id !== p?.id ? product : { ...product, hidden: p.hidden }
      )
    );
  }
  ...
  return (
    <div className="w-full h-fit min-h-screen bg-[url('/images/background.jpg')] bg-contain bg-repeat">
      {showCartPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[1001] p-8">
          <div className="flex flex-col justify-center items-center gap-2 bg-white bg-opacity-90 p-4 rounded-xl w-full">
            You've added{" "}
            <span className="font-retro text-xl bg-white px-2 w-[12rem] rounded-md text-center">
              {productAddedName}
            </span>{" "}
            to your cart!
            <button
              className="bg-edcYellow-60 text-white p-2 rounded-md w-[12rem]"
              onClick={() => navigate("/cart", { replace: true })}
            >
              Go to Cart!
            </button>
            <button
              className="bg-edcPurple-60 text-white p-2 rounded-md w-[12rem]"
      ...
      {showSortModal ? (
        <div className="fixed w-full h-[15rem] bg-edcPurple-80 text-white z-[1000] flex justify-around items-center bottom-0 border-t-2 border-white">
          <div
          ...
          <div className="flex flex-col justify-center items-center w-fit h-full ">
            <div
              className="w-full mb-2 flex justify-center items-center p-2 mt-4"
        ...
        <div
          className="text-xl text-white fixed bottom-0 z-[1000] w-full bg-edcPurple-80 flex justify-center items-center gap-2 py-2 border-t-2 border-white"
          onClick={() => setShowSortModal(true)}
        >
          Sort{" "}
          <IoIosArrowUp className="w-8 h-8 border-2 border-black rounded-lg" />
        </div>
      )}

      <div className="w-full min-h-screen flex flex-col items-center">
        {showProductForm && (
          <NewProductForm onClose={() => setShowProductForm(false)} />
        )}
        {showEditProductForm && (
          <EditProductForm
        ...
        <div className="w-full flex flex-wrap justify-center">
          ...
          {groupedInventory.map((series) => (
            <div
              key={series[0].series}
              className="w-screen text-white flex flex-wrap justify-center items-center"
            >
              <p className="text-white bg-edcPurple-80 text-2xl w-full text-center">
                {series[0].series}
              </p>
              <div className="w-full flex flex-wrap justify-center items-center bg-white bg-opacity-70 p-2 pb-[8rem]">
                {series.map((product) => (
                  <Frame
                    key={product.id}
                    additionalClass="w-[10rem] md:w-[15rem] h-fit relative m-2"
                  >
                    <p
                      className={`${
                        product.title.length > 11 ? "text-lg" : "text-2xl"
                      } text-edcPurple-80 w-full text-center`}
                    >
                      {product.title}
                    </p>
                    <div className="flex flex-col justify-between items-center h-full w-full">
                      ...
                      {user && (
                        <AdminButtons
                          ...
                          addPhotoToSeries={() =>
                            handleAddPhotoToSeries(product)
                          }
                        />
                      )}
                      <ShoppingButtons
