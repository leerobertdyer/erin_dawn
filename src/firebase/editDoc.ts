...
async function editProductDoc({
  ...
  try {
    ...
    console.log(
      `Document successfully updated with ID: ${id}, sold status: ${sold}`
    );
    return true;
  } catch (error) {
    console.error("Error updating document: ", error);
...
function buildPhotoUpdateData(
  photoData: Partial<IGeneralPhoto>
): Partial<IGeneralPhoto> {
  // Create an update object with only defined fields
  const updateData: Partial<IGeneralPhoto> = {};
  if (photoData.category !== undefined)
    updateData.category = photoData.category;
...
async function editPhotoDoc(
  photoData: Partial<IGeneralPhoto> & { id: string }
) {
  try {
