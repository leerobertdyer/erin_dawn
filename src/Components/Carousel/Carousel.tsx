...
export default function Carousel({ product, height, width, onClick }: ICarouselParams) {
    ...
    return (
        ...
        >
            ...
            {product.photos.length > 1 && <div className="w-full ">
                <div
                ...
                absolute top-4 z-[100]">
                    <IoIosArrowBack onClick={(e) => handleClick("left", e)} size={30} className="text-white bg-black bg-opacity-35 rounded-md" />
                    <IoIosArrowForward onClick={(e) => handleClick("right", e)} size={30} className="text-white bg-black bg-opacity-35 rounded-md" />
                </div>
            </div>}

            {/* Product image carousel */}
            {product.photos.length > 0  && (
                <div className={`${height ?? 'h-full'} ${width ?? 'w-full'} rounded-md overflow-hidden border-2 border-black relative`}>
                    {product.hidden && (
