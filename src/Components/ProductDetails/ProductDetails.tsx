...
export default function ProductDetails({ product, handleCloseProductDetails }: IProductDetails) {
    ...
    return (<div
        ...
        <div className="flex flex-col justify-between h-full max-w-[35rem] mx-auto border-edcPurple-80 border-2 bg-edcBlue-10 items-center rounded-md ">
            <p className="text-3xl p-2 w-full bg-edcPurple-40 rounded-t-md text-white text-center border-b-2 border-black">"{product.title}"</p>
            <p className="text-xl p-2 w-full text-center">${product.price}<span className="text-xs">.00</span></p>
            <p className="text-lg p-6 w-[85%] bg-white rounded-md whitespace-pre-line max-h-[550px] overflow-y-auto border-2 border-black">{product.description}</p>
            <p className="text-lg px-2 my-2 w-[85%] text-center bg-white rounded-md border-2 border-black">Size: {product.size}</p>
            <p className="text-lg px-2 my-2 w-[85%] text-center bg-white rounded-md border-2 border-black">Dimensions: {product.dimensions}</p>
