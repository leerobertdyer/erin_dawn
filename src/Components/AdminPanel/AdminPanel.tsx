...
export default function AdminPanel() {
  ...
  async function handleMarkAsShipped(customerName: string, sessionId: string) {
    ...
    setUnshippedOrders(
      unshippedOrders.filter((order: any) => order.sessionId !== sessionId),
    );
  }
    ...
    <>
      ...
      <div
        className="
            ...
            p-[2rem] max-w-[60rem] border-4 border-black mb-8"
      >
        {/* Logout */}
        <div className="flex flex-col items-center justify-center w-full">
          <div className="relative bg-[url('/images/catMad.jpg')] bg-cover bg-center bg-no-repeat w-[20rem] h-[20rem] rounded-md mt-4">
            <button
              onClick={logout}
              className="absolute bottom-2 right-2 p-2 bg-rose-600 rounded-md text-white min-w-[8rem]"
            >
              Log Out
            </button>
          </div>
        ...
        >
          <Frame additionalClass="h-[27rem] w-full">
            ...
            <button className="px-2 bg-edcPurple-60 rounded-md text-white w-full">
              Send Email Campaign
            </button>
          </Frame>
        </div>
      ...
      <div className="flex flex-col items-center justify-center w-screen mb-[4rem]">
        <div className="h-fit bg-edcBlue-80 rounded-md p-4 w-[90%] sm:w-[30rem] md:w-[40rem] lg:w-[50rem]">
          ...
          <p className="text-white text-center">
            Total {currentYear} Website Sales: $
            {allOrders.reduce((acc, item) => {
          ...
          <h1 className="text-2xl bg-white rounded-md w-fit p-2 mx-auto my-2">
            Current Unshipped Orders
          </h1>
          {unshippedOrders.map((order: any) => {
            return (
              <div
                key={order.sessionId}
                className="bg-white flex flex-col items-center justify-center gap-2 p-2 text-center border-2 border-black rounded-md"
              >
                ...
                >
                  ...
                  <button
                    onClick={() => handleClickCopy(order.shippingAddressString)}
                    className="bg-edcBlue-80 text-white rounded-md p-2"
                  >
                    <IoIosCopy />
                  </button>
                  {showCopyMessage &&
                    addressCopied === order.shippingAddressString && (
                      <span className="absolute top-[-.4rem] right-[-5rem] bg-white p-2 rounded-md border-red-500 border-2">
                        Copied!
                      </span>
                    )}
                </p>
                ...
                <p className="text-wrap text-center">
                  Item Sold: "{order.itemsSold}"
                </p>
                <button
                  className="p-2 bg-rose-600 text-white rounded-md"
                  onClick={() =>
                    handleMarkAsShipped(order.customerName, order.sessionId)
                  }
                >
                  Mark as Shipped
                </button>
